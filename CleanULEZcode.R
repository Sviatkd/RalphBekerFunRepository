...
treatment_control_df <- panel_dfind %>%
  filter(treatment + control == 1,yr_qtr < 2023.1)
...
hetmodel <- lm(mean_value~did*Station+yr_qtr,data=treatment_control_df)
...
coefplot::coefplot(eventmodel, 
                   coefficients = qtr_coefs,innerCI=0, vcov= hcr_vcov,horizontal = TRUE) + scale_y_discrete(labels = function(x) {
...
stargazer(eventmodel,hetmodel,reducedmodel,
          se=list(sqrt(diag(hcr_vcov)),sqrt(diag(hcr_vcovh)),sqrt(diag(hcr_vcovr))),column.labels = c("Event Model", "Heterogeneous Model","Reduced Model"),add.lines = list(aic_row, bic_row),omit.stat = c("f", "ser"),
          header = FALSE,type = "text",title="Pollutant Levels(NO2),Station and Quarter FE,HC3(Jacknife)SE",omit = c("^Station", "yr_qtr"))
stargazer(eventmodel,hetmodel,reducedmodel,
